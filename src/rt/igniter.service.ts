...
export class IgniterService {
  ...
  /**
   * Notifica um usuário específico via Igniter.js
   */
  async notifyUser(userId: string, data: any): Promise<void> {
    try {
      // Usar o sistema de notificações do Igniter para enviar dados em tempo real
      // O Igniter gerencia automaticamente as conexões SSE ativas

      this.logger.debug(`🔔 Notificando usuário ${userId} via Igniter`);

      // Aqui seria a integração real com o Igniter.js
      // Por enquanto, vamos cachear a notificação para streams ativos
      const notificationKey = `notification:user:${userId}:${Date.now()}`;

      this.cache.set(notificationKey, {
        data,
        timestamp: Date.now(),
      ...
      this.logger.error(`❌ Erro ao notificar usuário ${userId}:`, error);
      throw error;
    }
  }

  /**
   * Notifica administradores de um tenant via Igniter.js
   */
  async notifyTenantAdmins(tenantId: string, data: any): Promise<void> {
    try {
      this.logger.debug(
        `🔔 Notificando admins do tenant ${tenantId} via Igniter`,
      );

      // Cachear notificação para admins do tenant
      const notificationKey = `notification:tenant:${tenantId}:admins:${Date.now()}`;
      ...
      this.logger.log(
        `✅ Notificação enviada para admins do tenant ${tenantId}`,
      );
    } catch (error) {
      this.logger.error(
        `❌ Erro ao notificar admins do tenant ${tenantId}:`,
        error,
      );
      throw error;
    }
  ...
  async getUserNotifications(userId: string): Promise<any[]> {
    ...
    // Buscar notificações do usuário específico
    for (const [key, entry] of this.cache.entries()) {
      if (
        key.startsWith(`notification:user:${userId}:`) &&
  ...
  /**
   * Obtém notificações pendentes para admins de um tenant
   */
  async getTenantAdminNotifications(tenantId: string): Promise<any[]> {
    const notifications: any[] = [];

    // Buscar notificações para admins do tenant
    for (const [key, entry] of this.cache.entries()) {
      if (
        key.startsWith(`notification:tenant:${tenantId}:admins:`) &&
        Date.now() - entry.timestamp < entry.ttl
      ) {
        notifications.push({
          id: key,
  ...
  markNotificationAsRead(notificationId: string): void {
    ...
    this.logger.debug(`📖 Notificação ${notificationId} marcada como lida`);
  }

  // ==================== NOTIFICAÇÕES POR FILA ====================

  /**
   * Notifica sobre mudanças no ticket atual de uma fila específica
   */
  async notifyQueueTicketChange(
    queueId: string,
    data: {
    ...
    try {
      this.logger.debug(`🎫 Notificando mudança de ticket na fila ${queueId}`);

      const notificationKey = `notification:queue:${queueId}:ticket:${Date.now()}`;

      this.cache.set(notificationKey, {
        data: {
          type: 'queue-ticket-changed',
          queueId,
        ...
        timestamp: Date.now(),
        ttl: 2 * 60 * 1000, // 2 minutos para notificações de fila
      });

      this.logger.log(
        `✅ Notificação de ticket enviada para fila ${queueId}: ${data.currentTicket}`,
      );
    } catch (error) {
      this.logger.error(
        `❌ Erro ao notificar mudança de ticket na fila ${queueId}:`,
        error,
      );
      throw error;
    }
  ...
  /**
   * Notifica sobre mudanças no status de uma fila
   */
  async notifyQueueStatusChange(
    ...
    try {
      this.logger.debug(`📊 Notificando mudança de status na fila ${queueId}`);

      const notificationKey = `notification:queue:${queueId}:status:${Date.now()}`;
      ...
      this.cache.set(notificationKey, {
        ...
        timestamp: Date.now(),
        ttl: 5 * 60 * 1000, // 5 minutos para status da fila
      });

      this.logger.log(
        `✅ Notificação de status enviada para fila ${queueId}: ${data.status}`,
      );
    } catch (error) {
      this.logger.error(
        `❌ Erro ao notificar status da fila ${queueId}:`,
        error,
      );
      throw error;
    }
  ...
  /**
   * Notifica sobre mudança na posição de um usuário específico na fila
   */
  async notifyUserQueuePosition(
    userId: string,
    queueId: string,
    data: {
      position: number;
    ...
    try {
      this.logger.debug(
        `📍 Notificando posição do usuário ${userId} na fila ${queueId}`,
      );

      const notificationKey = `notification:user:${userId}:queue:${queueId}:position:${Date.now()}`;

      this.cache.set(notificationKey, {
        data: {
          type: 'user-queue-position-changed',
          userId,
          queueId,
          position: data.position,
          estimatedWait: data.estimatedWait,
        ...
        timestamp: Date.now(),
        ttl: 3 * 60 * 1000, // 3 minutos para posições
      });

      this.logger.log(
        `✅ Notificação de posição enviada para usuário ${userId} na fila ${queueId}: posição ${data.position}`,
      );
    } catch (error) {
      this.logger.error(
  ...
  /**
   * Obtém notificações de uma fila específica (ticket sendo chamado)
   */
  async getQueueNotifications(queueId: string): Promise<any[]> {
    const notifications: any[] = [];

    // Buscar notificações de ticket da fila específica
    for (const [key, entry] of this.cache.entries()) {
      if (
        key.startsWith(`notification:queue:${queueId}:ticket:`) &&
        Date.now() - entry.timestamp < entry.ttl
      ) {
        notifications.push({
          id: key,
  ...
  /**
   * Obtém notificações de status de uma fila específica
   */
  async getQueueStatusNotifications(queueId: string): Promise<any[]> {
    const notifications: any[] = [];

    // Buscar notificações de status da fila específica
    for (const [key, entry] of this.cache.entries()) {
      if (
        key.startsWith(`notification:queue:${queueId}:status:`) &&
        Date.now() - entry.timestamp < entry.ttl
      ) {
        notifications.push({
          id: key,
  ...
  /**
   * Obtém notificações da posição de um usuário específico em uma fila
   */
  async getUserQueuePosition(userId: string, queueId: string): Promise<any[]> {
    const notifications: any[] = [];

    // Buscar notificações de posição do usuário na fila específica
    for (const [key, entry] of this.cache.entries()) {
      if (
        key.startsWith(
          `notification:user:${userId}:queue:${queueId}:position:`,
        ) &&
        Date.now() - entry.timestamp < entry.ttl
      ) {
        notifications.push({
  ...
  /**
   * Obtém todas as filas que um usuário está ouvindo
   */
  async getUserActiveQueues(userId: string): Promise<string[]> {
    const activeQueues = new Set<string>();

    // Buscar por padrões de notificações de fila do usuário
    for (const [key] of this.cache.entries()) {
      if (key.startsWith(`notification:user:${userId}:queue:`)) {
        // Extrair queueId do padrão: notification:user:userId:queue:queueId:position:timestamp
        const parts = key.split(':');
        if (parts.length >= 5) {
          activeQueues.add(parts[4]);
        }
      }
    }
  ...
  /**
   * Remove todas as notificações de uma fila específica (limpeza)
   */
  clearQueueNotifications(queueId: string): number {
    let removedCount = 0;

    for (const key of this.cache.keys()) {
      if (key.includes(`queue:${queueId}:`)) {
        this.cache.delete(key);
        removedCount++;
      }
    }
    ...
    this.logger.log(
      `🗑️ Limpeza da fila ${queueId}: ${removedCount} notificações removidas`,
    );
    return removedCount;
  }
}
